...
function IsNotNull() {
    let x = JSON.parse(localStorage.getItem("question"));
    if (x != "") {
        document.querySelector("h4").innerText = `Score: ${x.score}`;
        document.querySelector("img").setAttribute("src", x.img);
        btnA.innerText = x.btna;
        btnB.innerText = x.btnb;
        btnC.innerText = x.btnc;
        btnD.innerText = x.btnd;

        let countl = JSON.parse(localStorage.getItem("count"));
        document.querySelector("span").innerText = `Secondary: ${countl}`;
        x = setInterval(() => {
            let countl = JSON.parse(localStorage.getItem("count"));
            if (countl > 0) {
                countl--;
                localStorage.setItem("count", JSON.stringify(countl));
